from core.external.hits.sparse_matrix import HITSSparseMatrix, HITSSparseMatrixWikidata, IN_EDGES, OUT_EDGES
...
class WesHITSer(object):
    ...
    def _init_structures(self):
        self._matrix = self._init_matrix()
        i = 0
    ...
    def _init_matrix(self):
        return HITSSparseMatrix(edges_yielder=self._edges_yielder,
                                max_edges=self._max_edges)

    def _compute_hubs_and_auth(self):
        ref_to_edges = self._matrix._nodes_dict
        ...
        while self._iterations_performed <= self._maxiters:
            new_hubs, new_auths = self._perform_iteration(ref_to_edges=ref_to_edges)
            try:
    ...
    def _perform_iteration(self, ref_to_edges):
        new_hubs = array.array(_ARRAY_TYPE)
        new_auths = array.array(_ARRAY_TYPE)
        for a_node in self._matrix.yield_nodes():
            new_auth_score = 0.0
            for an_incoming_node in ref_to_edges[a_node][IN_EDGES]:  # _compute_new_auth()
                new_auth_score += self._hub_scores[self._node_positions[an_incoming_node]]
            new_auths.append(new_auth_score)
            new_hubs_score = 0.0
            for an_outgoing_edge in ref_to_edges[a_node][OUT_EDGES]:  # _compute_new_hub()
    ...
    def _convergence_reached(self, new_auths):
        ...
        if self._same_non_convergence_strike >= self._longst_possible_streak:
            raise UnlikelyConvergenceException(non_convergent_nodes)
...
class WesHITSerWikidata(WesHITSer):

    def __init__(self, edges_yielder, max_iters=400, epsilon=1.0e-8, max_edges=-1, longest_posible_streak=40):
        super().__init__(edges_yielder, max_iters, epsilon, max_edges, longest_posible_streak)
    ...
    def _produce_final_output(self):
        ...
        :return:
        """
        return [(HITSSparseMatrixWikidata.int_to_qid(a_node), self._auth_scores[a_position], self._hub_scores[a_position])
                for a_node, a_position in self._node_positions.items()]

    def _init_matrix(self):
        return HITSSparseMatrixWikidata(edges_yielder=self._edges_yielder,
                                        max_edges=self._max_edges)