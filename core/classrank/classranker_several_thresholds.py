...
class ClassRanker(object):
    ...
    def _calculate_classrank(self, classes_dict, raw_pagerank, thresholds_list):
        for a_class in classes_dict:
            ...
            for a_threshold in thresholds_list:
                ...
                classes_dict[a_class][KEY_THRESHOLDS][a_threshold][KEY_UNDER_T_CLASS_POINTERS] = {}
                    ...
                    if len(classes_dict[a_class][KEY_CLASS_POINTERS][a_p]) >= a_threshold:
                        for an_s in classes_dict[a_class][KEY_CLASS_POINTERS][a_p]:
                            ...
                            if an_s not in classes_dict[a_class][KEY_THRESHOLDS][a_threshold][KEY_INSTANCES]:
                                ...
                                classes_dict[a_class][KEY_THRESHOLDS][a_threshold][KEY_CLASSRANK] += raw_pagerank[an_s]  # It must be there! KeyError?
                    else:
                        under_threshold_props[a_threshold].append((a_p,classes_dict[a_class][KEY_CLASS_POINTERS][a_p]))

            for a_threshold in thresholds_list:
                for a_low_p in under_threshold_props[a_threshold]:
